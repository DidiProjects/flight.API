import { Pool } from 'pg'
import { ITargetAlertStateRepository, AlertWatermark } from './interfaces/ITargetAlertStateRepository'
...
export class TargetAlertStateRepository implements ITargetAlertStateRepository {
  ...
  async getWatermarks(routineId: string, fareType: string): Promise<Map<string, number>> {
    const { rows } = await this.db.query<{ flight_date: string | Date; notified_amount: string }>(
      `SELECT flight_date, notified_amount
    ...
    for (const r of rows) map.set(toDateStr(r.flight_date), Number(r.notified_amount))
    return map
  }
  ...
  async recordNotified(routineId: string, fareType: string, entries: AlertWatermark[]): Promise<Set<string>> {
    ...
    const { rows } = await this.db.query<{ flight_date: string | Date }>(
      `INSERT INTO target_alert_state (routine_id, flight_date, fare_type, notified_amount, notified_airline)
       SELECT $1, d::date, $2, a::numeric, ai
       FROM unnest($3::date[], $4::numeric[], $5::text[]) AS t(d, a, ai)
      ...
      [routineId, fareType, dates, amounts, airlines],
    )
